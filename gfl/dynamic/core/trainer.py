...
from gfl.dynamic.entity import runtime_config
from gfl.dynamic.utils.utils import LoggerFactory
from gfl.dynamic.exceptions.fl_expection import PFLException
from gfl.dynamic.core.strategy import OptimizerStrategy, LossStrategy, SchedulerStrategy
...
class TrainStrategy(object):
    ...
    def __init__(self, client_id, job_path=None, local_model_base_path=None):
        self.client_id = client_id
        self.fed_step = {}
        self.job_iter_dict = {}
        # self.job_path = JOB_PATH
        self.job_path = job_path
        self.local_model_base_path = local_model_base_path
    ...
    def _create_job_models_dir(self, client_id, job_id):
        ...
        :return:
        """
        local_model_dir = os.path.join(self.local_model_base_path, "models_{}".format(job_id), "models_{}".format(client_id))
    ...
    def _find_latest_aggregate_model_pars(self, job_id):
        ...
        :return:
        """
        job_model_path = os.path.join(self.local_model_base_path, "models_{}".format(job_id), "{}".format(AGGREGATE_PATH))
        if not os.path.exists(job_model_path):
            os.makedirs(job_model_path)
            init_model_pars_path = os.path.join(self.local_model_base_path, "models_{}".format(job_id),
                                                "init_model_pars_{}".format(job_id))
            first_aggregate_path = os.path.join(self.local_model_base_path, "models_{}".format(job_id), "tmp_aggregate_pars",
                                                "avg_pars_{}".format(0))
        ...
        if len(file_list) != 0:
            # 用length标记fed_step不太合理，文件名中最后标记了fed_step
            # return os.path.join(job_model_path, file_list[-1]), len(file_list)
            last_file_name = file_list[-1]
            fed_step = int(last_file_name.split("_")[-1]) + 1
            return os.path.join(job_model_path, file_list[-1]), fed_step
        return None, 0
...
class TrainNormalStrategy(TrainStrategy):
    ...
    def __init__(self, job, train_data, valid_data, fed_step, client_id, local_epoch, model, curve, job_path, local_model_base_path):
        super(TrainNormalStrategy, self).__init__(client_id, job_path, local_model_base_path)
        self.job = job
        self.train_data = train_data
        self.valid_data = valid_data
        self.job_model_path = os.path.join(os.path.abspath(""), "models_{}".format(job.get_job_id()))
        self.fed_step = fed_step
        self.aggregate_step = copy.deepcopy(fed_step)
    ...
    def _train(self, train_model, job_models_path, fed_step, local_epoch):
        ...
        while step < local_epoch:
            dataloader = torch.utils.data.DataLoader(self.train_data,
            ...
            for idx, (batch_data, batch_target) in enumerate(dataloader):
                ...
                sum_loss += loss.item()
                if idx % 10 == 0:
                    self.logger.info("fed_step: {}, local epoch: {}, step: {},train_loss: {}"
                                     .format(fed_step, step+1, idx, sum_loss))
                    sum_loss = 0.0
            ...
            self.logger.info("fed_step: {}, local epoch: {}, acc: {}".format(fed_step, step, accuracy))
    ...
    def _prepare_job_model(self, job, server_url=None):
        job_model_path = os.path.join(self.local_model_base_path, "models_{}".format(job.get_job_id()))
    ...
    def _prepare_job_init_model_pars(self, job, server_url):
        job_init_model_pars_dir = os.path.join(self.local_model_base_path,
    ...
    def _prepare_upload_client_model_pars(self, job_id, client_id, fed_avg):
        job_init_model_pars_dir = os.path.join(os.path.abspath(""), self.local_model_base_path,
    ...
    def _draw_curve(self):
        # 由于存在动态模型融合的情况所以需要修改绘制图像的epoch的值
        loss_x = range(0, len(self.loss_list))
        accuracy_x = range(0, len(self.accuracy_list))
        # 不需要动态模型融合的情况
...
class TrainDistillationStrategy(TrainNormalStrategy):
    ...
    def __init__(self, job, train_data, valid_data, fed_step, client_id, local_epoch, models, curve, job_path, local_model_base_path):
        super(TrainDistillationStrategy, self).__init__(job, train_data, valid_data, fed_step, client_id, local_epoch, models, curve, job_path, local_model_base_path)
    ...
    def _load_other_models_pars(self, job_id, fed_step):
        ...
        :return:
        """
        job_model_base_path = os.path.join(self.local_model_base_path, "models_{}".format(job_id))
        other_models_pars = []
        fed_step += 1
...
class TrainStandloneNormalStrategy(TrainNormalStrategy):
    ...
    def __init__(self, job, train_data, valid_data, fed_step, client_id, local_epoch, model, curve, job_path, local_model_base_path):
        super(TrainStandloneNormalStrategy, self).__init__(job, train_data, valid_data, fed_step, client_id, local_epoch, model, curve, job_path, local_model_base_path)
...
class TrainStandloneDistillationStrategy(TrainDistillationStrategy):
    ...
    def __init__(self, job, train_data, valid_data, fed_step, client_id, local_epoch, model, curve, job_path, local_model_base_path):
        super(TrainStandloneDistillationStrategy, self).__init__(job, train_data, valid_data, fed_step, client_id, local_epoch, model, curve, job_path, local_model_base_path)
    ...
    def train(self):
        while True:
            ...
            if other_model_pars is not None and connected_clients_num and self._calc_rate(len(other_model_pars),
                ...
                init_model_pars_dir = os.path.join(self.local_model_base_path, "models_{}".format(self.job.get_job_id()),
...
class TrainMPCNormalStrategy(TrainNormalStrategy):
    ...
    def __init__(self, job, train_data, valid_data, fed_step, client_ip, client_port, server_url, client_id, local_epoch, model, curve, job_path, local_model_base_path):
        super(TrainMPCNormalStrategy, self).__init__(job, train_data, valid_data, fed_step, client_id, local_epoch, model, curve, job_path, local_model_base_path)
    ...
    def train(self):
        while True:
            # 初始化模型的融合次数，即fed_step
            self.fed_step[self.job.get_job_id()] = 0 if self.fed_step.get(
                self.job.get_job_id()
            ) is None else self.fed_step.get(
                self.job.get_job_id()
            )
            self.aggregate_step[self.job.get_job_id()] = 0 if self.aggregate_step.get(
                self.job.get_job_id()
            ) is None else self.aggregate_step.get(
                self.job.get_job_id()
            )
            # 判断当前的融合次数是否已经等于总的融合次数以及当前融合次数是否为空，满足上述条件则表示刚好完成任务，停止训练任务
            if self.fed_step.get(self.job.get_job_id()) is not None and self.fed_step.get(
                    self.job.get_job_id()) == self.job.get_epoch():
                self.logger.info("job_{} completed, final accuracy: {}".format(self.job.get_job_id(), self.acc))
                requests.put("/".join(
                    [self.server_url, "offline", "%s" % self.client_ip, "%s" % self.client_port, "%s" % self.client_id]
                ), data=None)
                if self.curve is True:
                    self._draw_curve()
                break
            # 当前的融合次数部位空以及当前融合次数是否大于总的融合次数，满足则表示早已经训练完成
            elif self.fed_step.get(self.job.get_job_id()) is not None and self.fed_step.get(
                    self.job.get_job_id()) > self.job.get_epoch():
                self.logger.warning("job_{} has completed, final accuracy: {}".format(self.job.get_job_id(), self.acc))
                # 从中心节点删除已完成任务的节点记录
            ...
            aggregate_file, fed_step = self._find_latest_aggregate_model_pars(self.job.get_job_id())
            """
            服务端通知客户端是否下发模型，收到该通知则表示可以进行下一步
            """
            # None表示服务端没有通知是否下发模型
            if DynamicClientConfig.get_server_is_dispatch() is not None:
                # False表示服务端通知该节点不下发模型
                if DynamicClientConfig.get_server_is_dispatch() is False:
                    self.aggregate_step[self.job.get_job_id()] = self.aggregate_step[self.job.get_job_id()] - 1
                    # 每个训练节点都不需要加载模型
                    DynamicClientConfig.set_need_load_model(False)
                else:
                    DynamicClientConfig.set_need_load_model(True)
                # 还原服务器通知客户端的标记
                DynamicClientConfig.set_server_is_dispatch(None)

            if aggregate_file is not None and self.aggregate_step.get(self.job.get_job_id()) != fed_step:
                # 获取模型文件路径
                job_models_path = self._create_job_models_dir(self.client_id, self.job.get_job_id())
                # 如果模型上传经过融合，需要加载中心节点下发的新模型
                if DynamicClientConfig.get_need_load_model():
                    self.logger.info("load {} parameters".format(aggregate_file))
                    new_model = self._load_job_model(self.job.get_job_id(), self.job.get_train_model_class_name())
                    # 申请获取模型的文件锁
                ...
                self.aggregate_step[self.job.get_job_id()] = fed_step
                self.logger.info("job_{} is training, Aggregator strategy: {}".format(
                    self.job.get_job_id(),
                    self.job.get_aggregate_strategy())
                )
                train_start_time = int(time.time())
                # 模型训练
                self.acc, loss = self._train(
                    self.model, job_models_path,
                    self.fed_step.get(self.job.get_job_id()),
                    self.local_epoch
                )
                ...
                """以下部分是经过修改后的代码，用于进行动态模型融合方式的验证"""
                # 通知中心节点实时更新所需要的等待时间长短
                requests.post(
                    "/".join(
                        [self.server_url, "notify_capacity", "%s" % self.client_id,
                         "%s" % (train_end_time - train_start_time)]
                    ), data=None
                )

                # 在这进行处理进行动态模型的融合，即只有好的模型才会上传到中心节点进行模型的融合
                if self.acc <= self.max_acc:
                    self.logger.info("accuracy is decline, can not be aggregated")
                    # 通知中心节点当前节点不上传模型
                    requests.post("/".join(
                        [self.server_url, "aggregate_request", "%s" % self.client_id, "none_aggregate"]
                    ), data=None)
                else:
                    self.max_acc = self.acc
                    # 通知中心节点当前节点上传模型
                    response = requests.post("/".join(
                        [self.server_url, "aggregate_request", "%s" % self.client_id, "aggregate"]
                    ), data=None)
                    if response and response.status_code == 200 and response.text:
                        ...
                        try:
                            ...
                            self.aggregate_step[self.job.get_job_id()] = fed_step - 1
                            # 每个训练节点都不需要加载模型
                            DynamicClientConfig.set_need_load_model(False)
                        # 正常上传模型
                        if response_dict["data"] == "upload":
                            # 从本地获取模型文件
                            files = self._prepare_upload_client_model_pars(
                                self.job.get_job_id(),
                                self.client_id,
                                self.fed_step.get(self.job.get_job_id())
                            )
                            ...
                            self.logger.info("upload model cost %s s" % (time.time() - upload_start_time))
                        elif response_dict["data"] == "timeout":
                            self.aggregate_step[self.job.get_job_id()] = self.aggregate_step[self.job.get_job_id()] - 1
                            # 每个训练节点都不需要加载模型
                            DynamicClientConfig.set_need_load_model(False)
                    pass
...
class TrainMPCDistillationStrategy(TrainDistillationStrategy):
    ...
    def __init__(self, job, train_data, valid_data, fed_step, client_ip, client_port, server_url, client_id, local_epoch, model, curve, job_path, local_model_base_path):
        super(TrainMPCDistillationStrategy, self).__init__(job, train_data, valid_data, fed_step, client_id, local_epoch, model, curve, job_path, local_model_base_path)
        self.client_ip = client_ip
        self.client_port = client_port
        self.server_url = server_url
        ...
        while True:
            ...
            if other_model_pars is not None and self._calc_rate(len(other_model_pars),
                ...
                self.acc, loss = self._train_with_distillation(self.model, other_model_pars, self.local_epoch,
                                                               os.path.join(self.local_model_base_path,
                ...
                job_model_client_path = os.path.join(self.local_model_base_path, "models_{}".format(self.job.get_job_id()),
                                                     "models_{}".format(self.client_id))
