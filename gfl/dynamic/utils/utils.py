...
# LOG_DIR = os.path.join(os.path.abspath("."), "res")
...
class ModelUtils(object):
    ...
    @staticmethod
    def get_init_model_from_remote(server_url, init_model_path, job_id):
        response = requests.get("/".join([server_url, "init_model/{}".format(job_id)]))
        response_data = response.content.decode("utf-8")
        init_model_filename = os.path.join(init_model_path, "init_model_{}.py".format(job_id))
        with open(init_model_filename, "w") as model_f:
            model_f.write(response_data)
