...
from gfl.normal.core.strategy import WorkModeStrategy
from gfl.normal.core.job_manager import JobManager
from gfl.normal.utils.utils import LoggerFactory
from gfl.normal.entity.runtime_config import WAITING_BROADCAST_AGGREGATED_JOB_ID_LIST, CONNECTED_TRAINER_LIST
            ...
            if f.find("models_") != -1:
                ...
                if one_model_par_files and len(one_model_par_files) != 0:
                    ...
                    if last_model_par_file_num > fed_step:
                        ...
                        # 在这经常会遇到EOFError的问题, 找不到原因, 故采用异常捕获的方式去解决问题
                        try:
...
class FedAvgAggregator(Aggregator):
    ...
    def aggregate(self):
        ...
        for job in job_list:
            ...
            # print("fed_step: {}, self.fed_step: {}, job_model_pars: {}"
            #       .format(fed_step, self.fed_step.get(job.get_job_id()), job_model_pars))
