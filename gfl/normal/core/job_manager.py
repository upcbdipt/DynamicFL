...
from gfl.normal.entity.job import Job
from gfl.normal.exceptions.fl_expection import PFLException
from gfl.normal.utils.utils import JobUtils, LoggerFactory
...
class JobManager(object):
    ...
    def generate_job(self, work_mode=WorkModeStrategy.WORKMODE_STANDALONE,
        ...
        with lock:
            ...
            job = Job(None, JobUtils.generate_job_id(), inspect.getsourcefile(model),   # type: ignore
                      model.__name__, fed_strategy, epoch,  distillation_alpha=distillation_alpha, l2_dist=l2_dist) # type: ignore
    ...
    def submit_job(self, job, model):
        ...
        with lock:
            ...
            job_model_dir = os.path.join(self.base_model_path, "models_{}".format(job.get_job_id()))
