...
from gfl.normal.core import communicate_server
from gfl.normal.core.aggregator import FedAvgAggregator
from gfl.normal.core.validator import Validator
from gfl.normal.utils.utils import LoggerFactory, CyclicTimer
...
class FLClusterServer(FLServer):
    ...
    def __init__(self, federate_strategy, ip, port, api_version, data=None, job_path=None, base_model_path=None):
        ...
        if data:
            self.validator = Validator(job_path, base_model_path, data)
        else:
            self.validator = None
